...
const Main = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      if (PokemonFetchtype.length) {
        setFilteredData(
          PokemonFetchtype?.filter(
            (v) =>
              v.pokemon.name.toLowerCase().includes(searchItem.toLowerCase()) &&
              v.type?.includes(selectedPokemonType)
          )
        );
      } else {
        setFilteredData(
          pokemonData?.results?.filter((v) =>
  ...
  }, [pokemonData, searchItem, selectedPokemonType]);
    ...
    <div className="max-w-7xl mx-auto px-4 py-10">
      {loading ? (
        ...
        <>
          ...
          {isfilteredData ? (
            <p className="text-center text-lg text-gray-700">Loading Pokémon...</p>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {(PokemonFetchtype.length ? PokemonFetchtype : filteredData)?.map(
                (item) => {
